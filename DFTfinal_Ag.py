...
defcoord = [[0, 0.25, 0.25], [0, -0.25, -0.25], 
            [-0.5, 0.6303997102, -0.1303997102], [-0.5, 0.3696002898, 0.1303997102], 
            [0.2392005796, 0.1303997102, -0.3696002898], 
            [0.7607994204, -0.1303997102, -0.6303997102]]
...
while(not(vconv)):
    ...
    while(not(relax)):
        ...
        if(os.path.isfile(relaxinname)):
            while(not(os.path.isfile(alloyfolder+"Relaxed"))):
                time.sleep(5)
        else:
            ...
            relaxinput = pwscf.PWInput(control = {"calculation": "relax", "prefix": alloyname+"_Relax", "outdir":"./tmp/", 
                                                  "pseudo_dir":"./pseudos/", "etot_conv_thr":0.0001, "forc_conv_thr":0.01},
